...
- Present a map.
- Tap on the map to select a place and inspect its details.
- Request the current, daily and hourly weather forecast for the selected place.
- Request the weather warnings issued for the selected place and draw their coverage areas on the map.
- Use Jetpack Compose with a model-view architecture (the `WeatherViewModel` holds all the SDK logic and state, the composables only render it).
