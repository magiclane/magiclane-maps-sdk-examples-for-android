...
import com.magiclane.sdk.examples.routesimulation.databinding.RoadblockLayoutBinding
...
class MainActivity : AppCompatActivity(), SoundUtils.ITTSPlayerInitializationListener {
    ...
    // Latest system bar / display cutout insets. Updated from the window insets listener rather
    // than read synchronously, because the insets are not available immediately and change on
    // rotation (a synchronous read during onConfigurationChanged still returns the pre-rotation
    // values).
    ...
    // so it can be dismissed when the simulation ends.
     ...
     * and the back key both dismiss it; [onNavigationEnded] takes care of the simulation ending
     * while it is open.
     */
