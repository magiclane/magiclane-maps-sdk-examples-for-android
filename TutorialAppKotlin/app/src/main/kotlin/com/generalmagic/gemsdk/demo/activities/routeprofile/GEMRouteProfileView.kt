...
import com.generalmagic.gemsdk.demo.activities.mainactivity.controllers.BaseUiRouteController
...
import com.generalmagic.gemsdk.models.Path
...
object GEMRouteProfileView {
    ...
    private var mSurfacesTypes = mutableMapOf<TSurfaceType, ArrayList<CSectionItem>>()
    private var mRoadsTypes = mutableMapOf<TRoadType, ArrayList<CSectionItem>>()
    private var mSteepnessTypes = mutableMapOf<Int, ArrayList<CSectionItem>>()

    private val KHighlightPathsColor = TRgba(239, 38, 81, 255)
    ...
    fun init(routeProfileView: RouteProfileView, mapView: MapView) {
        ...
        when (CommonSettings().getUnitSystem()) {
            TUnitSystem.EImperialUK,
            TUnitSystem.EImperialUS -> {
...
//
    ...
    fun getElevationChartPinImage(width: Int, height: Int): Bitmap? {
        return Utils.getImageAsBitmap(GemIcons.Other_UI.Search_Results_Pin.value, width, height)
    }
    ...
    fun onTouchElevationChart(evt: Int, x: Double) {
        mMapView.let { m_mapView ->
            ...
            if ((evt == TTouchChartEvent.EDown.ordinal) || (evt == TTouchChartEvent.EMove.ordinal)) {
                ...
                ImageDatabase().getImageById(GemIcons.Other_UI.Search_Results_Pin.value)
                    ?.let { image ->
                ...
                if ((mPrevTouchXMeters == x_m) && (evt == TTouchChartEvent.EDown.ordinal)) {
                    ...
                    settings.setOptions(
                        THighlightOptions.EHO_ShowContour.value
                            or THighlightOptions.EHO_Overlap.value
                            or THighlightOptions.EHO_NoFading.value
                    )
    ...
    private fun pointInEnvelope(rect: TRect, x: Int, y: Int): Boolean {
        return ((x >= rect.x()) &&
            (x <= rect.right()) &&
            (y >= rect.y()) &&
            (y <= rect.bottom()))
...
//        val controller = CAppController.getInstance();
        mMapView.let { m_mapView ->
            ...
            m_mapView.preferences()?.setMapViewPerspective(TMapViewPerspective.EMVP_2D, false)
...
//                setAutomaticZoomToRoute(false) TODO
                m_mapView.centerOnDistRoute(mRoute, distBegin, distEnd, TRect(), Animation())
            }
...
//            yardsOrFeet =
//                if (unitsSystem == TUnitSystem.EImperialUK) (miles * MILES_TO_YARDS) else (miles * MILES_TO_METERS * METERS_TO_FT)
        }
            ...
            return when (buttonType) {
                ...
                TElevationProfileButtonType.EElevationAtDeparture.ordinal -> {
                    Utils.getImageAsBitmap(
                        GemIcons.Other_Engine.WaypointFlag_PointStart.value,
                        width,
                        height
                    )
                }
                ...
                TElevationProfileButtonType.EElevationAtDestination.ordinal -> {
                    Utils.getImageAsBitmap(
                        GemIcons.Other_Engine.WaypointFlag_PointFinish.value,
                        width,
                        height
                    )
                }
                ...
                TElevationProfileButtonType.EMinElevation.ordinal -> {
                    Utils.getImageAsBitmap(
                        GemIcons.Other_UI.HeightProfile_Lowest_Point.value,
                        width,
                        height
                    )
                }
                ...
                TElevationProfileButtonType.EMaxElevation.ordinal -> {
                    Utils.getImageAsBitmap(
                        GemIcons.Other_UI.HeightProfile_Highest_Point.value,
                        width,
                        height
                    )
                }
...
//        if (m_view && m_mapView && routeTerrainProfile)
        routeTerrainProfile?.let { terrainProfile ->
            ...
            val image = ImageDatabase().getImageById(GemIcons.Other_UI.Search_Results_Pin.value)
            image?.let {
                landmark.setImage(image)
                mHighlightedLmkList.add(landmark)
    ...
    private fun getSurfaceColor(type: TSurfaceType): Int {
        return when (type) {

            TSurfaceType.EAsphalt -> {
                TRgba(127, 137, 149, 255).value()
            }

            TSurfaceType.EPaved -> {
                TRgba(212, 212, 212, 255).value()
            }

            TSurfaceType.EUnpaved -> {
                TRgba(157, 133, 104, 255).value()
            }

            TSurfaceType.EUnknown -> {
                TRgba(0, 0, 0, 255).value()
            }
    ...
    private fun getSurfaceName(type: TSurfaceType): String {
        return when (type) {
    ...
    private fun zoomToRoute() {
        mMapView.preferences()?.setMapViewPerspective(TMapViewPerspective.EMVP_2D, false)
    ...
    private fun getWayColor(type: TRoadType): Int {
        return when (type) {
            TRoadType.EMotorways -> {
                TRgba(242, 144, 99, 255).value()
            }

            TRoadType.EStateRoad -> {
                TRgba(242, 216, 99, 255).value()
            }

            TRoadType.ERoad -> {
                TRgba(153, 163, 175, 255).value()
            }

            TRoadType.EStreet -> {
                TRgba(175, 185, 193, 255).value()
            }

            TRoadType.ECycleway -> {
                TRgba(15, 175, 135, 255).value()
            }

            TRoadType.EPath -> {
                TRgba(196, 200, 211, 255).value()
            }

            TRoadType.ESingleTrack -> {
                TRgba(166, 133, 96, 255).value()
            }
        }
    }
...
//                    }
    ...
    private fun getWayName(type: TRoadType): String {
        return when (type) {
            TRoadType.EMotorways -> {
            ...
            for (item in mSteepnessTypes) {
                ...
                if (auxIndex < 0) {
                    return when (item.key) {
                        0 -> { // < -16
                            TRgba(4, 120, 8, 255).value()
                        }

                        1 -> { // [-16, -10]
                            TRgba(38, 151, 41, 255).value()
                        }

                        2 -> { // [-10, -7]
                            TRgba(73, 183, 76, 255).value()
                        }

                        3 -> { // [-7, -4]
                            TRgba(112, 216, 115, 255).value()
                        }

                        4 -> { // [-4, -1]
                            TRgba(154, 250, 156, 255).value()
                        }

                        5 -> { // [-1, 1]
                            TRgba(255, 197, 142, 255).value()
                        }

                        6 -> { // [1, 4]
                            TRgba(240, 141, 141, 255).value()
                        }

                        7 -> { // [4, 7]
                            TRgba(220, 105, 106, 255).value()
                        }

                        8 -> { // [7, 10]
                            TRgba(201, 73, 72, 255).value()
                        }

                        9 -> { // [10, 16]
                            TRgba(182, 43, 42, 255).value()
                        }

                        10 -> { // > 16
                            TRgba(164, 16, 15, 255).value()
                        }
    ...
    fun getSteepnessImage(index: Int, width: Int, height: Int): Bitmap? {
        ...
        return when (imageType) {
            TSteepnessImageType.EUp -> {
                Utils.getImageAsBitmap(GemIcons.Other_UI.SteepnessUp.value, width, height)
            }

            TSteepnessImageType.EDown -> {
                Utils.getImageAsBitmap(GemIcons.Other_UI.SteepnessDown.value, width, height)
            }

            TSteepnessImageType.EPlain -> {
                Utils.getImageAsBitmap(GemIcons.Other_UI.SteepnessPlain.value, width, height)
            }
