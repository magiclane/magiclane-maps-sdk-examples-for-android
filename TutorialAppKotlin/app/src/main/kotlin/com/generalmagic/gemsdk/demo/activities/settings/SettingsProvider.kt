...
object SettingsProvider {
    ...
    init {
        ...
        setNames()
        setDefaultValues()
        GEMSdkCall.execute { setCallbacks() }
    }
    ...
    private fun setCallbacks() {
            ...
            if (m_service != null) {
                if (value) {
                    ...
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.ETrafficService, bUseMobileDataForTraffic.second
                    )
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.EMapDataService, bUseMobileDataForMapAndWikipedia.second
                    )
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.ETerrainService,
                        bUseMobileDataForTerrainAndSatellite.second
                    )
                } else {
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.ETrafficService, value
                    )
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.EMapDataService, value
                    )
                    CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                        TServiceGroupType.ETerrainService, value
                    )
                }
            }
        }
        ...
        val useMobileDataForMapAndWikipedia = { value: Boolean ->
            if (m_service != null) {
                CommonSettings().setAllowOffboardServiceOnExtraChargedNetwork(
                    TServiceGroupType.EMapDataService,
                    value
                )
                if (value) {
                    val bUseMobileDataForTerrainAndSatellite =
