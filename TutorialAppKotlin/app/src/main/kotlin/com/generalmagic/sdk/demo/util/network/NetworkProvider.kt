...
class NetworkProviderImpl : NetworkProvider() {
    ...
    fun onNetworkConnectionTypeChanged(
        ...
        val connectResult = if (type.value < 0) {
            SdkError.KNoConnection
        } else SdkError.KNoError

        mConnected = (connectResult == SdkError.KNoError)
        networkType = type

        var gemNetworkType = ENetworkType.EFree
        if (type == NetworkManager.TConnectionType.TYPE_MOBILE) {
            gemNetworkType = ENetworkType.EExtraCharged
        }
    ...
    fun getNetworkName(connected: Boolean, networkType: ENetworkType): String {
        ...
        return when (networkType) {
            ENetworkType.EFree -> "Wi-Fi"
            ENetworkType.EExtraCharged -> "Mobile Data"
        }
    }
}
