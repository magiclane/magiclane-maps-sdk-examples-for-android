...
class NavTopPanelController(context: Context, attrs: AttributeSet?) :
    ...
    private fun updateNavigationInfo(navInstr: NavigationInstruction?) {
        ...
        if (turnDetails != null &&
            (turnDetails.getInfoType() == ETurnInfoType.ETI_WaypointEvent) &&
            (
                (turnDetails.getEvent() == ETurnWaypointType.ETW_Stop.value) ||
                    (turnDetails.getEvent() == ETurnWaypointType.ETW_Intermediate.value)
                )
        ) {
    ...
    private fun pickTrafficEvent(
        ...
        if (navInstr.getNavigationStatus() != SdkError.KNoError.value) return null
