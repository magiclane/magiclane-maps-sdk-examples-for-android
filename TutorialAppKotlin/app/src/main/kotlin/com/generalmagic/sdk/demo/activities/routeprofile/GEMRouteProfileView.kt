...
import com.generalmagic.sdk.demo.activities.mainactivity.controllers.BaseUiRouteController
import com.generalmagic.sdk.demo.activities.mainactivity.controllers.common.PickLocationController
...
object GEMRouteProfileView {
    ...
    fun init(routeProfileView: RouteProfileView, mapView: MapView) {
        ...
        when (CommonSettings().getUnitSystem()) {
            EUnitSystem.EImperialUK,
            EUnitSystem.EImperialUS -> {
                ...
                if ((mPrevTouchXMeters == x_m) && (evt == TTouchChartEvent.EDown.ordinal)) {
                    ...
                    settings.setOptions(
                        EHighlightOptions.EHO_ShowContour.value
                            or EHighlightOptions.EHO_Overlap.value
                            or EHighlightOptions.EHO_NoFading.value
                    )
...
//        val controller = CAppController.getInstance();
        mMapView.let { m_mapView ->
            ...
            m_mapView.preferences()?.setMapViewPerspective(EMapViewPerspective.EMVP_2D, false)
...
//            yardsOrFeet =
//                if (unitsSystem == EUnitSystem.EImperialUK) (miles * MILES_TO_YARDS) else (miles * MILES_TO_METERS * METERS_TO_FT)
        }
...
//        if (m_view && m_mapView && routeTerrainProfile)
    ...
    private fun getSurfaceColor(type: ESurfaceType): Int {
        return when (type) {
            ...
            ESurfaceType.EUnpaved -> {
                Rgba(157, 133, 104, 255).value()
            }

            ESurfaceType.EUnknown -> {
    ...
    private fun zoomToRoute() {
        mMapView.preferences()?.setMapViewPerspective(EMapViewPerspective.EMVP_2D, false)
    ...
    private fun getWayColor(type: ERoadType): Int {
        return when (type) {
            ERoadType.EMotorways -> {
                Rgba(242, 144, 99, 255).value()
            }

            ERoadType.EStateRoad -> {
