...
class FlyToArea(context: Context?, attrs: AttributeSet?) : FlyController(context, attrs) {
	...
	private val search = object : SearchServiceWrapper() {
		...
		override fun onSearchCompleted(reason: Int) {
			...
			val value = results[0]
			val area = GEMSdkCall.execute { value.getContourBox() } ?: return
			...
			GEMSdkCall.execute {
				...
				val option = if (area.isEmpty())
					THighlightOptions.EHO_ShowLandmark
				else
					THighlightOptions.EHO_ShowContour

				mainMap.centerOnRectArea(area, -3, TXy(), animation)
				mainMap.activateHighlight(arrayListOf(value), option.value)
			}
		}
	}
...
open class FlyToInstr(context: Context?, attrs: AttributeSet?) : FlyController(context, attrs) {
		...
		override fun onCompleteCalculating(reason: Int) {
			...
			if (isFlyToRoute) {
				GEMSdkCall.execute {
					...
					mainMap.getPreferences()?.routes()?.add(route, true)

					mainMap.centerOnRoute(route, animation)
				}
			} else {
				...
				GEMSdkCall.execute {
					...
					mainMap.centerOnRouteInstruction(instruction, -1, animation)
				}
			}
		}
	}
	...
	override fun doStart() {
		...
		val from = Landmark("Gheorghe Lazar", QualifiedCoordinates(45.65131, 25.60493))
		val to = Landmark("Bulevardul Grivitei", QualifiedCoordinates(45.65272, 25.60674))
...
class FlyToTraffic(context: Context?, attrs: AttributeSet?) : FlyController(context, attrs) {
	private val routing = object : RouteServiceWrapper() {
		...
		override fun onCompleteCalculating(reason: Int) {
			...
			GEMSdkCall.execute {
				...
				mainMap.centerOnRouteTrafficEvent(trafficEvent, -1, animation)
			}
		}
	}
	...
	override fun doStart() {
		...
		val from = Landmark("London", QualifiedCoordinates(51.50732, -0.12765))
		val to = Landmark("Maidstone", QualifiedCoordinates(51.27483, 0.52316))
