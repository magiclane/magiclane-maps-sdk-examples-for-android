...
class Utils {
	companion object {
		...
		fun getTimeText(
			timeInSeconds: Int, bForceHours: Boolean = false,
		...
		fun getDistText(
			mmeters: Int, unitsSystem: TUnitSystem,
			bHighResolution: Boolean = false, bCapitalizeUnit: Boolean = false
		): Pair<String, String> {
			var meters = mmeters
			var distance = ""
			var unit: String
		...
		fun getFormattedWaypointName(
			...
			val bContainsStreetName = nameContainsText(streetName) ||
				(bContainsStreetNumber && (nameParts.isNotEmpty()) && (nameParts[0].indexOf(
					streetName
				) > 0))
