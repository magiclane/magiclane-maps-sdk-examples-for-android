...
package com.generalmagic.gemsdkdemo
...
import com.generalmagic.gemsdkdemo.activities.searchaddress.SearchAddressActivity
import com.generalmagic.gemsdkdemo.controllers.*
...
import com.generalmagic.gemsdkdemo.util.StaticsHolder.Companion.gemMapScreen
import com.generalmagic.gemsdkdemo.util.StaticsHolder.Companion.getMainActivity
import com.generalmagic.gemsdkdemo.util.StaticsHolder.Companion.getMainMapView
import com.generalmagic.gemsdkdemo.util.StaticsHolder.Companion.getNavViewDrawerLayout
import com.generalmagic.gemsdkdemo.util.network.NetworkManager
import com.generalmagic.gemsdkdemo.util.network.NetworkProviderImpl
...
class MainActivity : BaseActivity() {
	...
	private val appNavListener = NavigationView.OnNavigationItemSelectedListener { item ->
		// handle menu action
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		nav_view.setNavigationItemSelectedListener(appNavListener)
		...
		gem_surface.onMainViewCreated = { mainView: View? ->
			...
			gem_surface?.onMainViewNotFollowingPosition = {
	...
	private fun selectLayout(layoutId: Int) {
		...
		GEMSdkCall.execute {
			val prefs = mainMapView?.getPreferences() ?: return@execute
		...
		when (layoutId) {
			...
			R.id.tutorial_twotiledViews -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_twotiledviews, contentMain
				).twoTiledViewsController
			}

			R.id.tutorial_multiple -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_multiplemaps, contentMain
			...
			R.id.tutorial_route_ab -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_route_ab, contentMain
				).routeAbController
			}

			R.id.tutorial_route_abc -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_route_abc, contentMain
				).routeAbcController
			}

			R.id.tutorial_route_custom -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_route_custom, contentMain
				).routeCustomController
			}

			R.id.tutorial_predef_sim -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_predef_sim, contentMain
				).predefSimController
			}

			R.id.tutorial_predef_nav -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_predef_nav, contentMain
				).predefNavController
			}

			R.id.tutorial_predef_ptnav -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_predef_ptnav, contentMain
				).predefPtNavController
			}

			R.id.tutorial_custom_ptnav -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_custom_ptnav, contentMain
				).customPtNavController
			}

			R.id.tutorial_custom_sim -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_custom_sim, contentMain
				).customSimController
			}

			R.id.tutorial_custom_nav -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_custom_nav, contentMain
				).customNavController
			}
			...
			R.id.tutorial_canvasDrawerCam ->{
				currentController = layoutInflater.inflate(
					R.layout.tutorial_drawercam, contentMain
				).canvasDrawerCamController
			}

			R.id.tutorial_flyto_coords -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_flyto_coords, contentMain
				).flyToCoordsController
			}

			R.id.tutorial_flyto_area -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_flyto_area, contentMain
				).flyToAreaController
			}

			R.id.tutorial_flyto_instr -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_flyto_instr, contentMain
				).flyToInstrController
			}

			R.id.tutorial_flyto_route -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_flyto_route, contentMain
				).flyToRouteController
			}

			R.id.tutorial_flyto_traffic -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_flyto_traffic, contentMain
			...
			R.id.tutorial_wiki -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_wiki, contentMain
				).wikiController
			}

			R.id.tutorial_custom_url -> {
				currentController = layoutInflater.inflate(
					R.layout.tutorial_custom_url, contentMain
