...
package com.generalmagic.gemsdkdemo.activities.publictransport
...
import com.generalmagic.gemsdkdemo.activities.RouteInstructionItem
...
class TRouteSegmentItem {
	...
	var m_backgroundColor = TRgba(255, 255, 255, 255).value()
	var m_foregroundColor = TRgba(0, 0, 0, 255).value()
	var m_name = ""
...
class TRouteItem {
	...
	var m_fare = ""
	var m_warning = ""
	var m_departureTime = Time()
	var m_arrivalTime = Time()
...
class Helper {
	companion object {
			...
			for (nSegmentIndex in 0 until nSegmentsCount) {
				...
				if (routeSegment.getSegmentType() == ERouteSegmentType.ESTPublicTransport) {
					...
					if (departureDelayInSeconds != 0) {
						if (departureDelayInSeconds < 0) {
							...
							routeDescriptionItem.m_stationEarlyTime.format(
								Utils.getUIString(StringIds.eStrXTimeEarly), tmp
							)
						} else {
							timeText = Utils.getTimeText(departureDelayInSeconds)
							val tmp = String.format("%s %s", timeText.first, timeText.second)
							...
							routeDescriptionItem.m_stationLaterTime.format(
								Utils.getUIString(StringIds.eStrXTimeLate), tmp
							)
						}
					} else
						if (routeSegment.toPTRouteSegment()
					...
					val distanceText = getDistText(
						...
						CommonSettings().getUnitSystem(), true
					)
					routeDescriptionItem.m_distanceToNextStation.format(
						"%s %s", distanceText.first, distanceText.second
					)
		...
		fun getIconId(type: ETransitType): Int {
			when (type) {
				ETransitType.ETTBus -> {
					return GemIcons.PublicTransport.PublicTransport_Bus.value
				}
				ETransitType.ETTUnderground -> {
					return GemIcons.PublicTransport.PublicTransport_Underground.value
				}
				ETransitType.ETTRailway -> {
					return GemIcons.PublicTransport.PublicTransport_Train.value
				}
				ETransitType.ETTTram -> {
					return GemIcons.PublicTransport.PublicTransport_Tram.value
				}
				ETransitType.ETTWaterTransport -> {
					return GemIcons.PublicTransport.PublicTransport_Water.value
				}
				ETransitType.ETTOther -> {
					return GemIcons.PublicTransport.PublicTransport_Other.value
				}
				ETransitType.ETTWalk -> {
					return GemIcons.PublicTransport.PublicTransport_Walk.value
				}
				else -> {
