...
import com.generalmagic.gemsdkdemo.activities.RouteDescriptionActivity.Companion.showRouteDescription
import com.generalmagic.gemsdkdemo.activities.publictransport.PublicTransportRouteDescriptionActivity.Companion.showPTRouteDescription
...
abstract class BaseUiRouteController(context: Context?, attrs: AttributeSet?) :
	...
	private val routing = object : RouteServiceWrapper() {
		...
		override fun onCompleteCalculating(reason: Int) {
			...
			GEMSdkCall.execute {
				...
				if (mainRoute != null)
					mainMap.centerOnRoute(mainRoute, Animation())
	...
	private fun clearRoutes(): Boolean {
		...
		val routes = StaticsHolder.getMainMapView()?.getPreferences()?.routes() ?: return false
		if (routes.size() == 0)
			return false
		routes.clear()
...
class RouteAbc(context: Context?, attrs: AttributeSet?) : BaseUiRouteController(context, attrs) {
	override fun doStart() {
		doStart(
			arrayListOf(
				Landmark("Frankfurt am Main", QualifiedCoordinates(50.11428, 8.68133)),
				Landmark("Karlsruhe", QualifiedCoordinates(49.0069, 8.4037)),
				Landmark("Munich", QualifiedCoordinates(48.1351, 11.5820))
			)
		)
	}
}
