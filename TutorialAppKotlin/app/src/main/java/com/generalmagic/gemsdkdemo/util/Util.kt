...
class Util {
	companion object {
		...
		fun createBitmap(
			img: LaneImageRef?, width: Int, height: Int,
			bkColor: TRgba? = null,
		...
		fun createBitmap(
			img: AbstractGeometryImageRef?, width: Int, height: Int,
			...
			val byteArray = GEMHelper.imageToBitmap(
				img, width, height,
				activeInnerColor, activeOuterColor, inactiveInnerColor, inactiveOuterColor
			)
