...
class NetworkManager(val context: Context) {
	...
	var onConnectionTypeChangedCallback: (
		type: TConnectionType, proxyType: Proxy.Type, proxyHost: String, proxyPort: Int
	) -> Unit = { _, _, _, _ -> }
			...
			GEMSdkCall.execute {
				if (config != null) {
					onConnectionTypeChangedCallback(
						connectionType, config.proxyType, config.proxyHost, config.proxyPort
					)
				} else {
					onConnectionTypeChangedCallback(
						connectionType, Proxy.Type.DIRECT, "", -1
					)
				}
			}
		}
