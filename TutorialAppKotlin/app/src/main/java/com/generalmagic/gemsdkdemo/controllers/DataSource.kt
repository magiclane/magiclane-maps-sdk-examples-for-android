...
import com.generalmagic.gemsdkdemo.activities.ChapterLISIAdapter
import com.generalmagic.gemsdkdemo.activities.GenericListActivity
import com.generalmagic.gemsdkdemo.activities.ListItemStatusImage
...
open class BasicSensorsActivity : GenericListActivity() {
	...
	open class DataItemViewModel(
		val type: EDataType, private val lastData: HashMap<EDataType, IData?>
	) : ListItemStatusImage() {
...
class SensorsListActivity : BasicSensorsActivity() {
	var currentDataSource: SenseDataSource? = null
	var listener = object : SenseDataSourceListener() {
		...
		override fun onNewData(pDataSource: SenseDataSourceRef, type: EDataType) {
			val willAdd = lastData[type] == null
			lastData[type] = pDataSource.getLatestData(type)
	...
	fun doStart() {
		...
		val currentDataSource = GMDataSourceFactory.createLive()
...
class DirectCamActivity : BaseActivity() {
	...
	private var currentDataSource: SenseDataSource? = null
	private var listener = object : SenseDataSourceListener() {
		override fun onNewData(pDataSource: SenseDataSourceRef, type: EDataType) {
			val iData = pDataSource.getLatestData(EDataType.Camera)?: return
			
			val data = ICamera(iData)
			val configs = data.getCameraConfiguration()?: return
...
class CanvasDrawerCam(context: Context, attrs: AttributeSet) : AppLayoutController(context, attrs){
	...
	private val cameraListener = object : SenseDataSourceListener() {
		override fun onNewData(pDataSource: SenseDataSourceRef, type: EDataType) {
			GEMSdkCall.execute {
				val iData = pDataSource.getLatestData(EDataType.Camera)?: return@execute

				val data = ICamera(iData)
				val buffer = data.getDirectBuffer()?: return@execute 
				val configs = data.getCameraConfiguration()?: return@execute
...
//				}
				...
				drawer?.uploadFrame(buffer, configs.frameWidth(), configs.frameHeight())
			}
		}
	}
