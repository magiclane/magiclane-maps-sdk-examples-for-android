...
import com.generalmagic.gemsdkdemo.util.Utils.Companion.getTimeText
import com.generalmagic.gemsdkdemo.util.Utils.Companion.getUIString
...
class NavTopPanelController(context: Context?, attrs: AttributeSet?) :
	...
	private fun updateTrafficEvent(trafficEvent: RouteTrafficEvent?) {
		...
		val distanceToTrafficPair = getDistText(
			distance, CommonSettings().getUnitSystem(), true
		)
		...
		if (!trafficEvent.isRoadblock()) {
			if (bInsideTrafficEvent) {
				if (trafficEvent.getLength() > 0) {
					val nRemainingTimeInsideTrafficEvent = (trafficEvent.getDelay() *
						nRemainingDistanceInsideTrafficEvent) / trafficEvent.getLength()
				...
				val trafficDistTextPair = getDistText(
					trafficEvent.getLength(), CommonSettings().getUnitSystem(), true
				)
	...
	private fun updateTrafficPanel() {
		...
		if (currentAlarmedMarker != null) {
			...
			traffic_panel.background =
				context?.resources?.getDrawable(
					R.drawable.bottom_rounded_white_button, null
				)
		...
		setPanelBackground(
			traffic_panel.background, trafficPanelBackgroundColor
		)
	...
	fun getTrafficEndOfSectionIcon(width: Int, height: Int): Bitmap? {
		...
		return Util.getImageIdAsBitmap(
			GemIcons.Other_UI.Traffic_EndOfSection_Square.value, width, height
		)
	}
