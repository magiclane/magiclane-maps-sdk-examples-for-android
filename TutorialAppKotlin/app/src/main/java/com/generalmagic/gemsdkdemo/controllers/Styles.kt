...
import com.generalmagic.gemsdkdemo.activities.*
...
class StylesActivity : GenericListActivity() {
	...
	private fun wrapList(list: ArrayList<ContentStoreItem>): ArrayList<ListItemStatusImage> {
		val styleId = getMainMapView()?.getPreferences()?.getMapStyleId()
	...
	private fun updateList(storeItems: ArrayList<ContentStoreItem>) {
		val adapter = ChapterLISIAdapter(wrapList(storeItems))
	...
	private fun onItemTapped(itUncasted: BaseListItem, holder: LISIViewHolder) {
		...
		val selectStyle = {
			GEMSdkCall.execute { getMainMapView()?.getPreferences()?.setMapStyleById(item.getId()) }
...
//								holder.refreshItemStatus(item)
	...
	private fun refreshChecked() {
		val adapter = list_view.adapter as ChapterLISIAdapter? ?: return
		val mainMap = getMainMapView() ?: return
		val styleId = mainMap.getPreferences()?.getMapStyleId() ?: return

		var changed = false
		for (sectionIndex in 0 until adapter.sectionCount) {
			val section = adapter.getSection(sectionIndex) as LISIViewHolder.Chapter<*>
