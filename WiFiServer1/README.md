...
- Acts as a WiFi server. It opens a TCP socket, advertises it on the local network via Network Service Discovery (DNS-SD) and sends navigation info (next turn icon as a PNG image, distance to next turn, next navigation instruction, remaining travel time and distance) as newline-delimited JSON messages to every connected WiFiClient1. Unlike the WiFiServer example, which sends a turn event id that the client maps to its own bundled icons, this example sends the rendered turn icon itself (like the BLEServer1 example does over GATT).

Run the WiFiClient1 example on another device connected to the same WiFi network to receive and display the navigation data.
