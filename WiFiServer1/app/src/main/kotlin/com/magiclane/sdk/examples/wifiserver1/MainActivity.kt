...
import com.magiclane.sdk.examples.wifiserver1.databinding.ActivityMainBinding
import com.magiclane.sdk.examples.wifiserver1.databinding.DialogLayoutBinding
...
class MainActivity : AppCompatActivity() {
     ...
     * (to bring newly connected clients up to date), hence @Volatile. The turn image is kept as
     * the Base64 PNG it is sent as; empty means "no active navigation".
     */
    @Volatile
    private var lastTurnImage = ""
     ...
     * WiFiClient1 apps (see [NavProtocol] for the message format).
     */
    private val navWifiServer by lazy {
        NavWifiServer(
            ...
            snapshotProvider = {
                buildList {
                    add(NavProtocol.turnImageMessage(lastTurnImage))
    ...
    // Encodes the turn bitmap as a Base64 PNG for the JSON protocol -- the WiFi analog of the
    // raw pixel bytes the BLEServer1 example streams over GATT.
    ...
    private val navigationListener: NavigationListener =
        NavigationListener.create(
            ...
            onNavigationInstructionUpdated = { instr ->
                ...
                // Check whether the turn icon changed and, if so, send the icon itself to the
                // WiFi clients (like the BLEServer1 example does with raw pixel data).
                ...
                if (!sameTurnImage.value) {
                    ...
                    lastTurnImage = newTurnImage?.toBase64Png() ?: ""
                    navWifiServer.broadcast(NavProtocol.turnImageMessage(lastTurnImage))
                }
    ...
    private fun onNavigationEnded(errorCode: ErrorCode = GemError.NoError) {
        runOnUiThread {
            ...
            // Reset the turn image and notify connected WiFi clients.
            lastTurnImage = ""
            navWifiServer.broadcast(NavProtocol.turnImageMessage(lastTurnImage))
        }
