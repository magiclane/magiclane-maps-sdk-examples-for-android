...
- Render thousands of campsites loaded from a bundled GeoJSON file.
- Cluster the markers natively into count "pill" bubbles at low zoom.
- Show per-type coloured pins (green = info-only, red = bookable) once clusters break apart, using two overlapping marker collections.
- Tap a cluster to list the campsites inside it; tap a row to highlight and center it.
