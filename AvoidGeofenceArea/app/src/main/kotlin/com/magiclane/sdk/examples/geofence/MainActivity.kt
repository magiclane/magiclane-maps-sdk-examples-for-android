...
class MainActivity : AppCompatActivity()
{
            ...
            when (errorCode)
            {
                GemError.NoError ->
                {
                    SdkCall.execute {
                        ...
                        {
                            gemSurfaceView.mapView?.presentRoute(routes[0], edgeAreaInsets = Rect(inset, inset, inset, inset))
                        }
                    }
                }
    ...
    private val loginProgressListener = ProgressListener.create(onCompleted = { error, _ ->
        ...
        {
            SdkCall.execute {
                geofenceAreas = arrayListOf(GeofenceArea(CircleGeographicArea(Coordinates(45.5950875, 25.6359825), 1000), "Area to avoid"))
