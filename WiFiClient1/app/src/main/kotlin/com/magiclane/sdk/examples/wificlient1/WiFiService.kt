...
/**
 * Service for managing the TCP connection and data communication with a WiFiServer1 discovered
...
class WiFiService : Service() {
    ...
    /**
     * Connects to the WiFiServer1 at the given address. The result and all further updates are
    ...
    private fun handleMessage(message: JSONObject) {
        ...
        when (message.optString(NavProtocol.KEY_TYPE)) {
            ...
            NavProtocol.TYPE_TURN_IMAGE -> {
                intent.putExtra(EXTRA_TYPE, DATA_TYPE_TURN_IMAGE)
                // The PNG bytes of the next-turn icon; empty when navigation ended.
                intent.putExtra(
                    EXTRA_DATA,
                    Base64.decode(message.optString(NavProtocol.KEY_IMAGE), Base64.NO_WRAP),
                )
            }
    ...
    companion object {
        ...
        const val ACTION_CONNECTED =
            "com.magiclane.sdk.examples.wificlient1.ACTION_CONNECTED"
        const val ACTION_DISCONNECTED =
            "com.magiclane.sdk.examples.wificlient1.ACTION_DISCONNECTED"
        const val ACTION_DATA_AVAILABLE =
            "com.magiclane.sdk.examples.wificlient1.ACTION_DATA_AVAILABLE"

        const val EXTRA_DATA = "com.magiclane.sdk.examples.wificlient1.EXTRA_DATA"
        const val EXTRA_TYPE = "com.magiclane.sdk.examples.wificlient1.EXTRA_TYPE"

        const val DATA_TYPE_INSTRUCTION = 0
        const val DATA_TYPE_DISTANCE = 1
        const val DATA_TYPE_TURN_IMAGE = 2
