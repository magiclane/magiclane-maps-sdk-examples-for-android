...
class UIDownloadingOnboardMapSimulationInstrumentedTests
{
    @Rule
    @JvmField
    ...
    fun checkViews(): Unit = runBlocking {
        navIdleResource = activityRes.getNavIdlingResource()
        IdlingRegistry.getInstance().register(navIdleResource)
    ...
    fun checkDownloadingViews(): Unit = runBlocking {
        ...
        onView(withId(R.id.flag_icon)).check(matches(isDisplayed()))
        onView(withId(R.id.download_progress_bar)).check(matches(isDisplayed()))
    }
