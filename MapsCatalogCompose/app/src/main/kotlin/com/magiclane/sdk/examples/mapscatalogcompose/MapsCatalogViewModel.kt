...
import com.magiclane.sdk.compose.components.contentstore.CatalogContinentUi
import com.magiclane.sdk.compose.components.contentstore.CatalogCountryUi
import com.magiclane.sdk.compose.components.contentstore.CatalogItemState
import com.magiclane.sdk.compose.components.contentstore.CatalogItemUi
...
import com.magiclane.sdk.compose.components.contentstore.MapUpdateCardMode
...
class MapsCatalogViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var onlineCountries by mutableStateOf<List<CatalogCountryUi>>(emptyList())
        private set

    var onlineContinents by mutableStateOf<List<CatalogContinentUi>>(emptyList())
        private set

    var localCountries by mutableStateOf<List<CatalogCountryUi>>(emptyList())
        private set

    /**
     * Version of the map data in use ("major.minor"), read from [MapDetails.mapVersion]
     */
    var mapsVersion by mutableStateOf("")
        private set

    var updateMode by mutableStateOf(MapUpdateCardMode.Check)
        private set
    ...
    var availableUpdateVersion by mutableStateOf("")
        private set
    ...
    // Must run on the SDK thread.
    private fun flagFor(code: String): ImageBitmap? = flagCache.getOrPut(code) {
    ...
    fun refreshLocalContent() {
        SdkCall.execute {
            ...
            for (item in items) {
                ...
                // completed maps are listed offline: an interrupted download belongs
        ...
        when (updateMode) {
            MapUpdateCardMode.Check ->
                if (isOnline) {
                    ...
                    errorMessage = getApplication<Application>().getString(R.string.update_check_requires_internet)
                }
    ...
    private fun checkForMapsUpdate() {
        SdkCall.execute {
            ...
            if (errorCode != GemError.NoError) {
                ...
                Util.postOnMain {
                    errorMessage = getApplication<Application>().getString(R.string.update_check_error, details)
                }
            }
        }
    }
    ...
    private fun applyMapsUpdate() {
        SdkCall.execute {
            ...
            if (!updateSilent) Util.postOnMain { updateMode = MapUpdateCardMode.Applying }
    ...
    private fun reportMapsUpdateApplied(applyError: Int) {
        ...
        if (applyError == GemError.NoError) {
            Util.postOnMain {
                infoMessage = getApplication<Application>().getString(R.string.maps_up_to_date)
            }
        } else {
            val details = GemError.getMessage(applyError, getApplication())
            Util.postOnMain {
                errorMessage = getApplication<Application>().getString(R.string.maps_update_apply_error, details)
    ...
    private fun startMapsUpdate(userRequested: Boolean = false, silent: Boolean = false) {
        ...
        if (!silent) {
            Util.postOnMain {
                when (updateMode) {
                    MapUpdateCardMode.Check -> updateMode = MapUpdateCardMode.Checking
        ...
        SdkCall.execute {
            ...
            if (contentUpdater == null || (createError != GemError.NoError && createError != GemError.Exist)) {
                if (!silent) Util.postOnMain { updateMode = idleUpdateMode() }
                return@execute
            }
                ...
                GemError.NoError -> {
                    ...
                    Util.postOnMain {
                        ...
                        if (userRequested && mode == MapUpdateCardMode.WaitingConnection) {
                            errorMessage = getApplication<Application>().getString(
                                R.string.update_check_requires_internet,
                            )
                            cancelMapsUpdate()
                        } else {
