...
class VoicesCatalogViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Fired (on the main thread) whenever any state the voices lists are built from
    ...
    var onlineGroups: List<CatalogVoiceGroupUi> = emptyList()
        private set

    var onlineContinents: List<CatalogVoiceContinentUi> = emptyList()
        private set

    var localGroups: List<CatalogVoiceGroupUi> = emptyList()
        private set
    ...
    // Must run on the SDK thread.
    private fun flagFor(code: String): Bitmap? {
    ...
    fun refreshLocalContent() {
        SdkCall.execute {
            ...
            // voices are listed offline: an interrupted download belongs to the
            // catalog tab until it finishes.
