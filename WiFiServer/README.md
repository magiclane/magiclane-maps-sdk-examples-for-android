...
- Acts as a WiFi server. It opens a TCP socket, advertises it on the local network via Network Service Discovery (DNS-SD) and sends navigation info (next turn event, distance to next turn, next navigation instruction) as newline-delimited JSON messages to every connected WiFiClient.

Run the WiFiClient example on another device connected to the same WiFi network to receive and display the navigation data.
