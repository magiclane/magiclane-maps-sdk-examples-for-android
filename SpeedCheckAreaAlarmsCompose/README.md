...
- Start simulated navigation along a route that crosses an average speed check area.
- Track the speed check areas (section control) along the route with an `AlarmService` and display an alarm panel (the `AlarmPanel` composable from the maps-compose library) with the entry gantry icon, the distance to the entry ("in 450 m") or to the exit ("out in 1.2 km") and the running average speed, shown in red when it exceeds the enforced limit.
- Highlight the entry and exit speed cameras (gantries) of the area on the map.
- Play TTS messages when approaching, entering and exiting the speed check area.
- Use Jetpack Compose.
