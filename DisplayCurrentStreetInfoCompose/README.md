...
- Present a map following the current GPS position.
- Display the current street and city names in a bottom panel (the `CurrentStreetPanel` composable from the maps-compose library), using the improved position data.
- Display the road speed limit in a round road sign (the `SpeedLimitSign` composable from the maps-compose library).
- Fall back to the nearest address ("Near …") when the position carries no city name.
- Use Jetpack Compose.
