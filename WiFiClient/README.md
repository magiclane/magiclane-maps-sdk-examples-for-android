...
WiFi client for the WiFiServer example. It discovers WiFiServer instances on the local network via Network Service Discovery (DNS-SD), connects to the selected one over TCP and displays the navigation instructions received from it (next turn icon, distance to the next turn and turn instruction text).
