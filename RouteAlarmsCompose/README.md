...
- Start simulated navigation between 2 given landmarks, along a route passing a safety camera.
- Track the safety cameras along the route with an `AlarmService` and display an alarm panel (the `AlarmPanel` composable from the maps-compose library) with the camera icon and the live distance to it.
- Highlight the alarmed camera and paint its field of view on the map.
- Play a text-to-speech warning when the camera is approached.
- Use Jetpack Compose.
