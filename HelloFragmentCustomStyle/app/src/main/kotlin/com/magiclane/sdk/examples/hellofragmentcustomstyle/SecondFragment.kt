...
class SecondFragment : Fragment() {
    ...
    private fun registerSdkListeners() {
        ...
        binding.gemSurfaceView.onDefaultMapViewCreated = { mapView ->
            ...
            updateFocusViewport()
            mapView.preferences?.let { applyCustomAssetStyle(it) }
        }
    ...
    // Loads and applies a custom map style from the app's assets.
    private fun applyCustomAssetStyle(mapViewPreferences: MapViewPreferences) {
        SdkCall.runSynced {
            val filename = "Basic_1_Oldtime_with_Elevation.style"
            val data = resources.assets.open(filename).readBytes()
            if (data.isEmpty()) return@runSynced
