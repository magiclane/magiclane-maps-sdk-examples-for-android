...
- Display the selected route on the map, with a horizontal routes list at the bottom
  (Magic Earth style); in landscape the list takes the left half of the screen and the
  route is presented in the right half.
- Open the full routes list (with Earlier/Later), the route description and the public
  transport settings; changed settings recalculate the routes.
